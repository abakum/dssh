...
12. Если невозможно подключиться к dssh-серверу командой `dssh -j host` но возможно с алиаса `jh` то подключаемся `dssh -J jh -j host`. Если на host.dssh.sshd запущен и dssh-сервер и sshd-сервер и невозможно подключиться `dssh -j host` то можно `dssh -J host.dssh.sshd -j :` или короче `dssh -J host.dssh.sshd`. Смотри [2.2](#2.2).
...
5. Если невозможно подключиться к dssh-серверу командой `dssh -9j host.dssh.sshd` но возможно через алиас `jh` то подключаемся `dssh -J jh,host.dssh.sshd -9j :` или короче `dssh -9J jh,host.dssh.sshd`. Возможность идентифицировать хост dssh-сервера по сертификату пользователя имеет WinSCP увы FileZilla этого не может. Но WinSCP не имеет возможности использовать цепочки посредников (jump host) это преодолевается локальным переносом -L 127.0.0.1:2290:последнийАдресвЦепочкеПосредников:последнийПортвЦепочкеПосредников плата за это необходимость подтверждения хостов sshd-серверов.
