...
7. Когда используется `dssh -0 X` это аналог `dssh -t X dssh -20` или для Windows 7 без Cygwin `dssh -t X dssh -80`.
...
6. Если в пунктах [4.2](#4.2)-[4.5](#4.5) заменить `-20` на `-80` то вместо telnet://host:5000 будет использоваться http&#65279;://host:8000 то есть будет запущен веб-сервер на порту 8000. Например `dssh -8 8000` или `dssh -80`. Это используется в `dssh -0` которая на Windows 7 без Cygwin вместо `dssh -20 .` запустит `dssh -80 .`. <div id=4.7>
...
8. Иначе чтоб подключиться через браузер к телнет-серверу 192.168.2.1:5000 [3.4](#3.4) через sshd-сервер `X` `dssh -H192.168.2.1:5000 -80 X` или короче `dssh -0H192.168.2.1: X`.
