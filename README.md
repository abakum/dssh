...
- y. локальная последовательная консоль на первом свободном порту USB. Или путь к последовательному порту на Linux как /dev/ttyUSB0. Или последовательный порт на Windows как COM1. Или суффикс порта от 0 до 9.
- X. IP или dns адрес или ssh-алиас или dssh-алиас `.` как сокращение алиаса `dssh` или dssh-алиас `:` как сокращение алиаса `ssh-j`.
- x. отсутствует зачит 127.0.0.1 или локальный IP или локальный алиас: `+` как `0.0.0.0` или `_` как последний сетевой интерфейс.
- host. X или x.
- port50. отсутствует или суффикс от 0 до 9 или TCP порт. Если port50 отсутствует то он 5000. Если суффикс от 0 до 9 то порт от 5000 до 5009.
- port80. суффикс порта 8000 от 0 до 9 или TCP порт. Если port80 отсутствует то он 8000. Если суффикс от 0 до 9 то порт от 8000 до 8009
...
- host. X или x.
- port22. отсутствует или суффикс от 0 до 9 или TCP порт. Если port22 отсутствует то он 2200. Если суффикс от 0 до 9 то порт от 2200 до 2209.
...
9. Вместо `ssh X` как `dssh X` и наоборот вместо `dssh :` `dssh -Z :` или `ssh :`  
...
9. Если WAN RouterOS это 192.168.1.2 то подключаемся `dssh -H:5000 admin@192.168.1.2` или короче `dssh -H: admin@192.168.1.2` это то же что и `dssh -NL127.0.0.1:5000:127.0.0.1:5000 admin@192.168.1.2` а потом `dssh -H:`.
10. Если запустить `ssh -NL127.0.0.1:5000:127.0.0.1:5000 admin@192.168.1.2` то `dssh -H:` сработает только однажды потому, что после закрытия `dssh -H:` статус `/port remote-access` останется подключенным. Это особенность ssh.
...
- port50. TCP порт.<div id=10.1>
1. `-z` то же что и `-U 9600` как в [1.1](#1.1).<div id=10.2>
2. `-22 x` то же что `-U 9600` + стартует RFC2217 телнет-сервер на `x:5002` как в [1.11](#1.11) + стартует RFC2217 телнет-клиент в том же окне.<div id=10.3>
...
7. `-u22 x` то же что и `-22 x` + `putty -telnet x -P 5002` как в [1.4](#1.4).<div id=10.8>
...
- port50. TCP порт.<div id=11.1>
1. `-z .` как в [10.1](#10.1) только на хосте с `dssh`. Как `ssh : dssh -z`.<div id=11.2>
2. `-20 .` или короче `-0` как в [10.2](#10.2) только на хосте с `dssh` + 127.0.0.1:5000 переносится на 127.0.0.1:5000 сервера.<div id=11.3>
...
10. `-s` как в [10.2](#10.2) стартует RFC2217 телнет-сервер на 127.0.0.1:5000.<div id=11.11>
...
15. `-s20 .` или короче `-s` как в [10.2](#10.2) + 127.0.0.1:5000 sshd-сервера X переносится на 127.0.0.1:5000 локального телнет-сервера с RFC2217.
