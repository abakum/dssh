...
#10.    Параметры где `x` отсутствует или локальный IP или локальный алиас типа: `_` или `*` или `+`, `X` это внешний IP или алиас или `.` или `:`, `Y` последовательная консоль на первом порту USB:
    ...
    8.  `-zu` то же что и `plink -serial Y -sercfg 9600,8,1,N,N`. Для Cygwin на Windows7 `putty -serial Y -sercfg 9600,8,1,N,N`.