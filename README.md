...
# 1.    Как использовать для доступа к локальной последовательной консоли:
    ...
    4.  Вместе с PuTTY для доступа к локальной последовательной консоли на первом найденном USB порту `dssh -uUU` (На Linux и Darwin используется plink если нет plink то `busybox microtty`.
    ...
    7.  Вместе с telnet для доступа к локальной последовательной консоли на первом найденном USB порту через telnet://127.0.0.1:2322 `dssh -zZ22`.
    ...
    15. Когда используется и -2 и : то RFC2217 telnet сервер запускается на стороне ssh сервера а порт указанный в -2 форвардится локально. Управление режимом последовательной консоли в этом случае ведётся не по протоколу RFC2217 а по ssh.
...
# 3.    Как использовать для доступа к удалённой последовательной консоли:
    ...
    2.  
    3.  Если на 192.168.0.2 запущен сервер RFC2217 telnet c доступом к последовательной консоли через порт 5000 например `dssh -25000 _` как в 1.12 или сервис ser2net или RouterOS с портом USB и адаптером USB2serial то с 192.168.0.3 можно подключиться командой `dssh -W192.168.0.2:5000` в режиме raw без управления режимом последовательной консоли или `dssh -H192.168.0.2:5000` в режиме RFC2217 c управлением режимом последовательной консоли.
    4.  Если на 192.168.0.1 запущен `dssh` и на 192.168.0.2 запущен сервер RFC2217 telnet c доступом к последовательной консоли через порт 5000 как в 3.3 то можно подключиться командами `dssh -W192.168.0.2:5000 :` или `dssh -H192.168.0.2:5000 :`. Как на 3.3 только отовсюду.
    5.  Если на 192.168.0.1 и 192.168.1.1 запущен `sshd` и на 192.168.0.2 запущен сервер RFC2217 telnet c доступом к последовательной консоли через порт 5000 как в 3.3 то присоединится к сессии можно командами `ssh -W192.168.0.2:5000 192.168.1.1` или `dssh -W192.168.0.2:5000 192.168.1.1` 
    6.  Если на хосте запущен `dssh` и на нём же запущен сервер RFC2217 telnet c доступом к последовательной консоли например `dssh -22` то можно подключиться отовсюду командами `dssh -W127.0.0.1:2322 :` или `dssh -W:2322 :` или `dssh -W2322 :` или `dssh -H127.0.0.1:2322 :` или `dssh -H:2322 :`.
