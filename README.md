...
# 2.    Как использовать для доступа к удалённой консоли по ssh:
    1.  При наличии на хосте Сети запускаем SSH-сервер командой `dssh` (Сервер будет ждать на 127.0.0.1:2222).
    ...
    3.  При отсутствии на хосте Сети запускаем SSH-сервер командами `dssh *` или `dssh _` или `dssh -d host:port` и сообщаем клиенту host:port. (Где * как в 1.9 а _ как 1.10).
...
# 3.    Как использовать для доступа к удалённой последовательной консоли:
    ...
    3.  Если на 192.168.0.1 запущен `dssh` и на 192.168.0.2 запущен сервер RFC2217 телнет c доступом к последовательной консоли через порт 5000 как в 3.2 то можно подключиться командами `dssh -W192.168.0.2:5000 :` или `dssh -H192.168.0.2:5000 :`. Как на 3.2 только отовсюду.
    4.  Если на 192.168.0.1 и 192.168.1.1 запущен `sshd` и на 192.168.0.2 запущен сервер RFC2217 телнет c доступом к последовательной консоли через порт 5000 как в 3.2 то присоединится к сессии можно командами `ssh -W192.168.0.2:5000 192.168.1.1` или `dssh -W192.168.0.2:5000 192.168.1.1` 
    5.  Если на хосте запущен `dssh` и на нём же запущен сервер RFC2217 телнет c доступом к последовательной консоли например `dssh -22` то можно подключиться отовсюду командами `dssh -W127.0.0.1:2322 :` или `dssh -W:2322 :` или `dssh -W2322 :` или `dssh -H127.0.0.1:2322 :` или `dssh -H:2322 :`.
    6.  Когда используется `dssh -22 :` или `dssh -88 :` то телнет или веб-сервер запускается на хосте SSH-сервера а порт указанный в -2 или -8 форвардится локально. Управление режимом последовательной консоли в этом случае ведётся не по протоколу RFC2217 а по ssh.
...
# 4.    Как использовать для совместного доступа к последовательной консоли (или консоли с интерпретатором команд) нескольких клиентов:
    1.  Запускаем на удалённом хосте SSH-сервер `dssh`.
...
# 6.    Как устроена авторизация:
    ...
    6.  Врочем вместо начала хэша комита можно использовать что-то попроще - переименовываем `dssh` в `ivanov` и посылаем Иванову. Он запускает SSH-сервер `ivanov` а мы подключаемтся как `dssh -l ivanov :`.
...
# 11.    Параметры для удалённого доступа:
    ...
    4.  `-Z22 :` то же что и `-22 :` см. 11.2 + стартует `telnet -e^Q 127.0.0.1 2322` учитывая форвардинг портов управление будет консолью на хосте сервера.
    5.  `-Z X` то же что и `ssh X` для Windows новом окне см. 2.8.
    6.  `-zZ X` то же что и `ssh X` для Windows в том же окне см. 2.8.
    7.  `-u22 :` то же что и `-22 :` + стартует `putty -telnet 127.0.0.1 -P 2322` учитывая форвардинг портов управление будет консолью на хосте сервера.
    ...
    11. `-88 :` то же что `-U 9600` + стартует веб-сервер на хосте SSH-сервера `127.0.0.1:8088` + локальный порт 127.0.0.1:8088 форвардится на порт 127.0.0.1:8088 сервера + стартует `chrome http:\127.0.0.1:8088` учитывая форвардинг портов управление будет консолью на хосте сервера.
    12. `-22 -88 :` то же что `-U 9600` + 127.0.0.1:2322 и 127.0.0.1:8088 форвардятся на 127.0.0.1:2322 и 127.0.0.1:8088 сервера + стартует RFC2217 телнет-сервер на `127.0.0.1:2322` + стартует веб-сервер на `127.0.0.1:8088` + стартует `chrome http:\127.0.0.1:8088` учитывая форвардинг портов управление будет консолью на хосте сервера.
    13.  `-22 X` то же что и 10.2 + 127.0.0.1:2322 SSH-сервера X форвардится на 127.0.0.1:2322 локального телнет-сервера с RFC2217. Это работает не только через хост с запущенным SSH-сервером `dssh`.
    14. `-H host:port X` 127.0.0.1:port форвардится SSH-сервером X на host:port + локальный телнет клиент по RFC2217 управляет через SSH-сервер X телнет-сервером с RFC2217 на host:port см. 9.9. Это работает не только через хост с запущенным SSH-сервером `dssh`. Отданная как в 11.13 через SSH-сервер X консоль может использоваться как 11.14 так и 11.2.
    15.  `-s22 :` то же что и 10.2 + 127.0.0.1:2322 сервера X форвардится на 127.0.0.1:2322 локального телнет-сервера с RFC2217. Это работает не только через хост с запущенным SSH-сервером `dssh`.
