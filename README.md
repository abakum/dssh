...
16. Чтоб поделится локальной последовательной консолью через sshd-сервер `X` `dssh -22 X`. Для подключения к этой консоли `dssh -H:2322 X` или без управления `dssh -W2322 X` или без управления и без перевода консоли в сырой (raw) режим `ssh -W127.0.0.1:2322 X`.<div id=1.17>
17. Чтоб поделится локальной последовательной консолью через dssh-сервер `dssh -s22 :`. Для подключения к этой консоли `dssh -H:2322 :` или без управления `dssh -W2322 :` или без управления и без перевода в сырой режим консоли `ssh -W127.0.0.1:2322 ssh-j`.
...
5.  Если клиент находится на хосте, для теста, подключаемся к dssh-серверу без посредников комндой `dssh .` или `dssh -j:` или `dssh -j x:port` или через посредника `dssh :`.<div id=2.6>
...
- X.    IP или dns адрес или ssh алиас.
1.  Как в [2.2](#2.2)-[2.6](#2.6) только добавив ключи `-z` `-U` `-H` `-2` `-8` из раздела [1](#1.1). Например `dssh -z :` или `dssh -H3 :`. Это похоже на `dssh : dssh -z` или `ssh -t ssh-j dssh -z`. Ключ `-t` переводит консоль в сырой режим. Вариант `dssh -z :` лучше протоколируется, зато вариант `ssh -t X dssh -z` позволяет подключаться к sshd-серверу `X`.<div id=3.2>
2.  Если на 192.168.2.1 запущен RFC2217 телнет-сервер c доступом к последовательной консоли через порт 5000 например `dssh -25000 _` как в [1.12](#1.12) или сервис ser2net или RouterOS с портом USB и адаптером USB2serial то с 192.168.2.2 можно подключиться `dssh -W_:5000` в сыром режиме без управления режимом последовательной консоли или `dssh -H_:5000` в режиме RFC2217 c управлением режимом последовательной консоли.
3.  Если на 192.168.2.2 запущен `dssh` а на 192.168.2.1 запущен RFC2217 телнет-сервер c доступом к последовательной консоли через порт 5000 как в [3.2](#3.2) то можно подключиться `dssh -W192.168.2.1:5000 :` без управления режимом консоли или `dssh -H192.168.2.1:5000 :`. Как в [3.2](#3.2) только отовсюду.
4.  Если на 192.168.2.2 запущен sshd-сервер `X` а на 192.168.2.1 запущен RFC2217 телнет-сервер c доступом к последовательной консоли через порт 5000 как в [3.2](#3.2) то присоединится к консоли можно `ssh -W192.168.2.1:5000 X` или `dssh -H192.168.2.1:5000 X`.<div id=3.5>
...
6.  Когда используется `dssh -22 :` или `dssh -88 :` то телнет или веб-сервер запускается на хосте dssh-сервера а порт указанный в `-2` или `-8` переносится локально. Управление режимом последовательной консоли в этом случае ведётся не по протоколу RFC2217 а по ssh с помощью эскэйп последовательности.
...
3.  Если LAN хоста с запущенными `dssh` это 192.168.0.1 а у устройства с RouterOS LAN это 192.168.0.2 то подключаемся `dssh -W192.168.0.2:5000 :` или `dssh -tW192.168.0.2:5000 admin@192.168.0.1` или `dssh -t admin@192.168.0.1 dssh -W192.168.0.2:5000`. Это похоже на `ssh -W192.168.0.2:5000 admin@192.168.0.1` только ssh не переводит консоль в сырой режим а dssh переводит.
...
1.  `-z :` как в [10.1](#10.1) только на хосте с `dssh`. Как `ssh ssh-j dssh -z`.<div id=11.2>
