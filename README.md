...
- host.   X или x.
- port.   TCP порт.
1.  Вместо PuTTY, plink [0.7](#0.7) для доступа к локальной последовательной консоли на первом USB порту `y` со скоростью 9600 `dssh -U9600` или `dssh -U9` или `dssh -UU` или `dssh -z`. Можно задать любую стартовую скорость на `y` а потом переключать! На Darwin 12.7.6 тоже работает. Для Linux нужно членство в группе dialout.<div id=1.2>
...
5.  Вместе с plink для доступа к `y` `dssh -zu`. Ключ `-z` в том же окне. С Сygwin на Windows 7 как [1.4](#1.4).<div id=1.6>
...
7.  Вместе с telnet для доступа к `y` через telnet://127.0.0.1:2320 `dssh -zZ`. Ключ `-z` в том же окне. С Сygwin на Windows 7 как [1.1](#1.1).<div id=1.8>
...
13. Когда используется `dssh -20 -88` это всё равно, что совместно `dssh -20` и `dssh -H:2320 -88` или коротко `dssh -H: -88`. Режим посредника.<div id=1.14>
...
16. Чтоб поделиться локальной последовательной консолью через sshd-сервер `X` `dssh -s20 X` или `dssh -s X`. Для подключения к этой консоли `dssh -H127.0.0.1:2320 X` или `dssh -H:2320 X` или `dssh -H: X` или коротко `dssh -HH X` или без управления и без перевода консоли в сырой (raw) режим `ssh -W127.0.0.1:2320 X`.<div id=1.17>
17. Чтоб поделиться локальной последовательной консолью через dssh-сервер `dssh -s20 :`, где алиас `:` это сокращение алиаса `ssh-j` вместо `X` из [1.16](#1.16) или коротко `dssh -s`. Для подключения к этой консоли `dssh -HH` или без управления `dssh -W2320 :` или без управления и без перевода консоли в сырой режим `ssh -W127.0.0.1:2320 :`.
...
8.  Так же как в [1.8](#1.8)-[1.12](#1.12) но с параметром `-Hcmd` для Windows или `-Hbash` для Linux. Например `dssh -Hcmd -20 :` или коротко `dssh -0Hcmd` или `dssh -Hbash -88 :`.
...
5.  Если на хосте запущен dssh-сервер и на нём же запущен  RFC2217 телнет-сервер c доступом к последовательной консоли например `dssh -20` то можно подключиться отовсюду `dssh -W127.0.0.1:2320 :` или `dssh -W:2320 :` или `dssh -W2320 :` или `dssh -H127.0.0.1:2320 :` или `dssh -H:2320 :` или коротко `dssh -HH`.
6.  Когда используется `dssh -20 :` или коротко `dssh -0` или `dssh -88 :` то телнет или веб-сервер запускается на хосте dssh-сервера а порт указанный в `-2` или `-8` переносится локально. Управление режимом последовательной консоли при `-8` ведётся не по протоколу RFC2217 а по ssh.
...
2.  Команда `dssh -2 2320 :` или `dssh -20 :` или коротко `dssh -0` запускает на удалённом хосте RFC2217 телнет-сервер с портом 2320. По умолчанию порт 2320 будет подключен к последовательной консоли на первом USB порту со скоростью 9600. Ключами `-H` `-U` можно указать другой последовательный порт и другую стартовую скорость.<div id=4.3>
3.  Присоединяемся к консоли на порту 2320 `dssh -H:2320 :` или `dssh -20 :` или коротко или `dssh -0` или с PuTTY `dssh -u0` или c telnet `dssh -Z0` или без управлением режимом консоли `dssh -W2320 :`. Таких подключений может быть несколько.<div id=4.4>
...
8.  Иначе как в [1.13](#1.13) `dssh -22 -88 :` или коротко `dssh -088`
...
2.  `-20 :` или коротко `-0` как в [10.2](#10.2) только на хосте с `dssh` + 127.0.0.1:2320 переносится на 127.0.0.1:2320 сервера.<div id=11.3>
3.  `-H host:port :` то же что и `dssh -W host:port :` только с управлением по RFC2217 или RFC1073 как в [3.5](#3.5).<div id=11.4>
4.  `-Z20 :` или коротко `-Z0` то же что и `-20 :` как в [11.2](#11.2) + `telnet -e^Q 127.0.0.1 2320` учитывая перенос портов управление будет консолью на хосте сервера.<div id=11.5>
5.  `-Z X` то же что и `ssh X` для Windows новом окне.<div id=11.6>
6.  `-zZ X` то же что и `ssh X` для Windows в том же окне.<div id=11.7>
7.  `-u20 :` или коротко `-u0` то же что и `-20 :` + `putty -telnet 127.0.0.1 -P 2320` учитывая перенос портов управление будет консолью на хосте сервера.<div id=11.8>
...
12. `-22 -88 :` или коротко `-088` то же что `-U 9600` + 127.0.0.1:2320 и 127.0.0.1:8080 переносятся на 127.0.0.1:2320 и 127.0.0.1:8088 сервера + стартует RFC2217 телнет-сервер на `127.0.0.1:2320` + стартует веб-сервер на `127.0.0.1:8088` + `chrome http:\127.0.0.1:8088` учитывая перенос портов управление консолью будет на хосте сервера.<div id=11.13>
13.  `-s20 X` или коротко `-s X` как в [10.2](#10.2) + 127.0.0.1:2320 sshd-сервера X переносится на 127.0.0.1:2320 локального телнет-сервера с RFC2217. Это работает не только c dssh-сервером.<div id=11.14>
...
15.  `-s20 :` или коротко `-s` как в [10.2](#10.2) + 127.0.0.1:2320 sshd-сервера X переносится на 127.0.0.1:2320 локального телнет-сервера с RFC2217.
