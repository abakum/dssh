...
#10.    Параметры: (где x отсутствует или локальный IP или локальный алиас типа: _ или * или + а X это внешний IP или алиас или . или :)
    1.  `-z` то же что и `-U 9600` см. 1.1
    2.  `-22 x` то же что `-U 9600` плюс запуск RFC2217 telnet сервера на `x:2322` см. 1.11 
    3.  `-Z` для Linux c busybox и microcom открывает первую найденную среди USB последовательную консоль на стартовой скорости 9600
    3.  `-Z22 x` то же что и `-22 x` а потом `telnet x 2322` для Windows в новом окне см. 1.6
    4.  `-zZ x` то же что и `-20 x` а потом `telnet x 2320` в том же окне.
    5.  `-u22 x` то же что и `-22 x` а потом `putty -telnet x -P 2322` для Windows старше 7 в новом окне см. 1.6
    6.  `-u` то же что и `-20 x` а потом `telnet x 2320` в том же окне.
