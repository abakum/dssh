...
# 3.    Как использовать для доступа к удалённой последовательной консоли:
    ...
    6. Когда используется `dssh -22 :` или `dssh -88 :` то telnet или web сервер запускается на хосте ssh сервера а порт указанный в -2 или -8 форвардится локально. Управление режимом последовательной консоли в этом случае ведётся не по протоколу RFC2217 а по ssh.
...
# 10.    Параметры для локального доступа где `x` отсутствует или локальный IP или локальный алиас: `_` или `*` или `+`, `Y` локальная последовательная консоль на первом свободном порту USB, `host` любой IP или адрес хоста или x, `port` TCP порт:
    1.  `-z` то же что и `-U 9600` см. 1.1
    2.  `-22 x` то же что `-U 9600` + стартует RFC2217 telnet сервер на `x:2322` см. 1.11 + стартует RFC2217 telnet клиент в том же окне.
    ...
    12.  `-22 -88 x` то же что `-U 9600` + стартует RFC2217 telnet сервер на `x:2322` + стартует web сервера на `x:8088` + стартует `chrome http:\x:8088`   

# 11.    Параметры для удалённого доступа где `X` это внешний IP или адрес или ssh алиас или `.` или `:` а `Y` удалённая последовательная консоль на первом свободном порту USB:
    1.  `-z :` то же что и 10.1 только на хосте с `dssh`.
    2.  `-22 :` то же что и 10.2 только на хосте с `dssh` + локальный порт 127.0.0.1:2322  форвардится на порт 127.0.0.1:2322 сервера.
    ...
    8.  `-u X` то же что и `putty X` для Windows новом окне см. 1.4. Для Linux то же что и `plink @X`.
    9.  `-zu X` то же что и `plink @X` см. 1.5.
    10.  `-22 .` то же что и 11.2 только на локальном хосте с `dssh` стартует RFC2217 telnet сервер на `127.0.0.1:2332` + локальный порт 127.0.0.1:2322  форвардится на порт 127.0.0.1:2332 см 2.5.
    ...
    12.  `-22 -88 :` то же что `-U 9600` + локальный порты 127.0.0.1:2322 и 127.0.0.1:8088 форвардятся на порты 127.0.0.1:2322 и 127.0.0.1:8088 сервера + стартует RFC2217 telnet сервер на `127.0.0.1:2322` + стартует web сервер на `127.0.0.1:8088` + стартует `chrome http:\127.0.0.1:8088` учитывая форвардинг портов управление будет консолью на хосте сервера.
