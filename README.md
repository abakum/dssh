...
# 3.    Как использовать для доступа к удалённой последовательной консоли:
    ...
    3.  Если на 192.168.0.2 запущен сервер RFC2217 telnet c доступом к последовательной консоли через порт 23 например `dssh -2 23` или сервис ser2net или RouterOS с портом USB и адаптером USB2serial то с 192.168.0.3 то можно подключиться командой `dssh -W192.168.0.2:23` в режиме raw или `dssh -H192.168.0.2:23` в режиме RFC2217.
