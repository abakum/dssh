...
1.  Как в [2.2](#2.2)-[2.6](#2.6) только добавив ключи `-z` `-U` `-H` `-2` `-8` из раздела [1](#1.1). Например `dssh -z :` или `dssh -H3 :`. Это похоже на `dssh : "dssh -z"` но с переводом терминала в raw режим.<div id=3.2>
