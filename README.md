...
# 3.    Как использовать для доступа к удалённой последовательной консоли:
    ...
    6. Когда используется `dssh -22 :` или `dssh -88 :` то телнет или web сервер запускается на хосте ssh сервера а порт указанный в -2 или -8 форвардится локально. Управление режимом последовательной консоли в этом случае ведётся не по протоколу RFC2217 а по ssh.
...
# 10.    Параметры для локального доступа:
    x.    отсутствует или локальный IP или локальный алиас: `_` или `*` или `+`.
    ...
    11.  `-88 x` то же что `-U 9600` + стартует web сервер на `x:8088` + стартует `chrome http:\x:8088`
    12.  `-22 -88 x` то же что `-U 9600` + стартует RFC2217 телнет сервер на `x:2322` + стартует web сервера на `x:8088` + стартует `chrome http:\x:8088`

# 11.    Параметры для удалённого доступа:
    X.  внешний IP или адрес или ssh алиас или `.` или `:`.
    ...
    11. `-88 :` то же что `-U 9600` + стартует web сервер на хосте dssh сервера `127.0.0.1:8088` + локальный порт 127.0.0.1:8088 форвардится на порт 127.0.0.1:8088 сервера + стартует `chrome http:\127.0.0.1:8088` учитывая форвардинг портов управление будет консолью на хосте сервера.
    12. `-22 -88 :` то же что `-U 9600` + 127.0.0.1:2322 и 127.0.0.1:8088 форвардятся на 127.0.0.1:2322 и 127.0.0.1:8088 сервера + стартует RFC2217 телнет сервер на `127.0.0.1:2322` + стартует web сервер на `127.0.0.1:8088` + стартует `chrome http:\127.0.0.1:8088` учитывая форвардинг портов управление будет консолью на хосте сервера.
    13. `-H host:port X` 127.0.0.1:port форвардится сервером X на host:port + локальный телнет клиент по RFC2217 управляет через сервер X телнет сервером с RFC2217 на host:port.