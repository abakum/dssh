...
# 1.    Как использовать для доступа к локальной последовательной консоли:
    ...
    11. Как в 1.9 но через telnet://0.0.0.0:23 - `dssh -223 0.0.0.0` или `dssh -223 *`.
    12. Как в 1.10 но через telnet://192.168.0.21:23 - `dssh -223 192.168.0.1` или  `dssh -223 _` (Где 192.168.0.2 это первый сетевой интерфейс).
    ...
    14. А dssh с ключом -H host:port работает как RFC2217 telnet клиент и может удалённо менять режимы работы последовательной консоли: baudRate - скорость бит/сек, dataBits - количесто бит в данных, parity - чётность, stopBits - количество стоповых бит, flowControl - управление потоком . По умолчанию это 9600,8,N,1,N.
...
# 3.    Как использовать для доступа к удалённой последовательной консоли:
    ...
    3.  Если на 192.168.0.2 запущен сервер RFC2217 telnet c доступом к последовательной консоли через порт 23 например `dssh -223 _` как в 1.12 или сервис ser2net или RouterOS с портом USB и адаптером USB2serial то с 192.168.0.3 можно подключиться командой `dssh -W192.168.0.2:23` в режиме raw без управления режимом последовательной консоли или `dssh -H192.168.0.2:23` в режиме RFC2217 c управлением режимом последовательной консоли.
    4.  Если на 192.168.0.1 запущен `dssh` и на 192.168.0.2 запущен сервер RFC2217 telnet c доступом к последовательной консоли через порт 23 как в 3.3 то можно подключиться командами `dssh -W192.168.0.2:23 :` или `dssh -H192.168.0.2:23 :`. Как на 3.3 только отовсюду.
    5.  Если на 192.168.0.1 и 192.168.1.1 запущен `sshd` и на 192.168.0.2 запущен сервер RFC2217 telnet c доступом к последовательной консоли через порт 23 как в 3.3 то присоединится к сессии можно командами `ssh -W192.168.0.2:23 192.168.1.1` или `dssh -W192.168.0.2:23 192.168.1.1` 
