...
8.  [Corey Minyard](https://github.com/cminyard/ser2net).
...
- host.   X или x.
- port.   TCP порт.
1.  Вместо PuTTY, plink [0.7](#0.7) для доступа к локальной последовательной консоли на первом USB порту `y` со скоростью 9600 `dssh -U9600` или `dssh -U9` или `dssh -UU` или короче `dssh -z`. Можно задать любую стартовую скорость на `y` а потом переключать! На Darwin 12.7.6 тоже работает. Для Linux нужно членство в группе dialout.<div id=1.2>
2.  Вместо PuTTY, plink для доступа к локальной последовательной консоли на порту `COM3` для Windows со скоростью 9600 `dssh -HCOM3` или короче `dssh -H3`.<div id=1.3>
...
6.  Вместе с telnet для доступа к `y` через telnet://127.0.0.1:2320 `dssh -Z`. Для Windows в новом окне. С Сygwin на Windows 7 как [1.1](#1.1).<div id=1.7>
7.  Вместе с telnet для доступа к `y` через telnet://127.0.0.1:2320 `dssh -zZ`. С Сygwin на Windows 7 как [1.1](#1.1).<div id=1.8>
8.  В окне веб браузера для доступа к `y` через http&#65279;://127.0.0.1:8088 - `dssh -8 8088` или короче `dssh -88`.<div id=1.9>
9.  Как в [1.8](#1.8) но через http&#65279;://192.168.1.1:8080 `dssh -8 8080 0.0.0.0` или `dssh -80 0.0.0.0` или короче `dssh -80 +`, где 192.168.1.1 это первый сетевой интерфейс.<div id=1.10>
10. Как в [1.9](#1.9) но через http&#65279;://192.168.2.1:8089 `dssh -8 8089 192.168.2.1` или `dssh -89 192.168.2.1` или короче `dssh -89 _`, где 192.168.2.1 это последний сетевой интерфейс.<div id=1.11>
11. Как в [1.9](#1.9) но через telnet://192.168.1.1:5000 `dssh -25000 0.0.0.0` или короче `dssh -25000 +`.<div id=1.12>
12. Как в [1.10](#1.10) но через telnet://192.168.2.1:5000 `dssh -25000 192.168.2.1` или короче `dssh -25000 _`.<div id=1.13>
13. Когда используется `dssh -20 -88` это всё равно, что совместно `dssh -20` и `dssh -H127.0.0.1:2320 -88` или короче `dssh -H: -88`. Режим посредника.<div id=1.14>
14. Клиент dssh c параметрами `-2 port x` работает и как RFC2217 телнет-клиент [1.15](#1.15) и как RFC2217 телнет-сервер типа [ser2net](#0.8) только без команд для модема.<div id=1.15>
15. Клиент dssh с параметрами `-H host:port` работает как RFC2217 телнет-клиент и может удалённо менять режимы работы последовательной консоли: baudRate - скорость бит/сек, dataBits - количесто бит в данных, parity - чётность, stopBits - количество стоповых бит. По умолчанию это 9600,8,N,1,N. Если host опущен то он 127.0.0.1. Если port опущен то он 2320. Если port от 0 до 9 то он от 2320 до 2329. Если `-HH` то это как  `-H: .`<div id=1.16>
16. Чтоб поделиться локальной последовательной консолью через sshd-сервер `X` `dssh -s20 X` или короче `dssh -s X`. Для подключения к этой консоли `dssh -H127.0.0.1:2320 X` или `dssh -H:2320 X` или `dssh -H: X` или короче `dssh -HH X` или без управления и без перевода консоли в сырой (raw) режим `ssh -W127.0.0.1:2320 X`.<div id=1.17>
17. Чтоб поделиться локальной последовательной консолью через dssh-сервер `dssh -s20 :`, где алиас `:` это сокращение алиаса `ssh-j` вместо `X` из [1.16](#1.16) или короче `dssh -s`. Ключ `-s`это сокращение `-s20 .` Для подключения к этой консоли `dssh -HH` или без управления `dssh -W2320 .` или без управления и без перевода консоли в сырой режим `ssh -W127.0.0.1:2320 .`. Вместо алиаса `:` можно использовать алиас `.` это сокращение алиаса `dssh` в этом случае если на хосте dssh-клиента не запущен dssh-сервер то будет использован алиас `:` для доступа к dssh-серверу через посредника ssh-j.com.
...
5.  Если клиент находится на хосте, подключаемся к dssh-серверу без посредников `dssh .` или `dssh -j:` или `dssh -j x:port` или через посредника `dssh :` .<div id=2.6>
...
7.  Вместе с ssh в отдельном окне `dssh -Z .` похоже на [1.6](#1.6) или вместе с ssh в том же окне `dssh -zZ .` похоже на [1.7](#1.7). Должно выполняться условие [6.8](#6.8). C Cygwin  на Windows 7 только как `dssh -zZ .`
8.  Так же как в [1.8](#1.8)-[1.12](#1.12) но с параметром `-Hcmd` для Windows или `-Hbash` для Linux. Например `dssh -Hcmd -20 .` или короче `dssh -0Hcmd` или `dssh -Hbash -88 .`.
...
5.  Если на хосте запущен dssh-сервер и на нём же запущен  RFC2217 телнет-сервер c доступом к последовательной консоли например `dssh -20` то можно подключиться отовсюду `dssh -W127.0.0.1:2320 .` или `dssh -W:2320 .` или `dssh -W2320 .` или `dssh -H127.0.0.1:2320 .` или `dssh -H:2320 .` или `dssh -H: .` или короче `dssh -HH`.
6.  Когда используется `dssh -20 .` или короче `dssh -0` или `dssh -88 .` то телнет или веб-сервер запускается на хосте dssh-сервера а порт указанный в `-2` или `-8` переносится локально. Управление режимом последовательной консоли при `-8` ведётся не по протоколу RFC2217 а по ssh.
...
2.  Команда `dssh -2 2320 .` или `dssh -20 .` или короче `dssh -0` запускает на удалённом хосте RFC2217 телнет-сервер с портом 2320. По умолчанию порт 2320 будет подключен к последовательной консоли на первом USB порту со скоростью 9600. Ключами `-H` `-U` можно указать другой последовательный порт и другую стартовую скорость.<div id=4.3>
3.  Присоединяемся к консоли на порту 2320 `dssh -H:2320 .` или `dssh -20 .` или короче или `dssh -0` или с PuTTY `dssh -u0` или c telnet `dssh -Z0` или без управлением режимом консоли `dssh -W2320 .`. Таких подключений может быть несколько.<div id=4.4>
...
5.  Другие клиенты в локальной сети хоста могут присоединиться к консоли `dssh -20 -j host`, где `host` это адрес запущенного dssh-сервера или на Windows `telnet host 2320`.<div id=4.6>
6.  Если в пунктах [4.2](#4.2)-[4.5](#4.5) заменить `-20` на `-88` то вместо telnet://host:2322 будет использоваться http&#65279;://host:8088 то есть будет запущен веб-сервер на порту 8088. Например `dssh -8 8088` или `dssh -88`.<div id=4.7>
...
7.  Иначе чтоб подключиться через браузер к телнет-серверу 192.168.2.1:5000 [3.3](#3.3) через dssh-сервер `dssh -H192.168.2.1:5000 -88 .`
8.  Иначе чтоб подключиться через браузер к телнет-серверу 192.168.2.1:5000 [3.4](#3.4) через sshd-сервер `X` `dssh -H192.168.2.1:5000 -88 X`
...
2.  `-20 .` или короче `-0` как в [10.2](#10.2) только на хосте с `dssh` + 127.0.0.1:2320 переносится на 127.0.0.1:2320 сервера.<div id=11.3>
3.  `-H host:port .` то же что и `dssh -W host:port .` только с управлением по RFC2217 или RFC1073 как в [3.5](#3.5).<div id=11.4>
4.  `-Z20 .` или короче `-Z0` то же что и `-20 .` как в [11.2](#11.2) + `telnet -e^Q 127.0.0.1 2320` учитывая перенос портов управление будет консолью на хосте сервера.<div id=11.5>
5.  `-Z X` то же что и `ssh X` для Windows новом окне.<div id=11.6>
6.  `-zZ X` то же что и `ssh X` для Windows в том же окне.<div id=11.7>
7.  `-u20 .` или короче `-u0` то же что и `-20 .` + `putty -telnet 127.0.0.1 -P 2320` учитывая перенос портов управление будет консолью на хосте сервера.<div id=11.8>
...
10. `-s` как в [10.2](#10.2) только для теста на локальном хосте с `dssh` + стартует RFC2217 телнет-сервер на 127.0.0.1:2330.<div id=11.11>
11. `-88 .` то же что `-U 9600` + стартует веб-сервер на хосте dssh-сервера `127.0.0.1:8088` + локальный порт 127.0.0.1:8088 переносится на порт 127.0.0.1:8088 сервера + `chrome http://127.0.0.1:8088` учитывая перенос портов управление консолью  будет на хосте сервера.<div id=11.12>
12. `-22 -88 .` или короче `-088` то же что `-U 9600` + 127.0.0.1:2320 и 127.0.0.1:8080 переносятся на 127.0.0.1:2320 и 127.0.0.1:8088 сервера + стартует RFC2217 телнет-сервер на `127.0.0.1:2320` + стартует веб-сервер на `127.0.0.1:8088` + `chrome http://127.0.0.1:8088` учитывая перенос портов управление консолью будет на хосте сервера.<div id=11.13>
13.  `-s20 X` или короче `-s X` как в [10.2](#10.2) + 127.0.0.1:2320 sshd-сервера X переносится на 127.0.0.1:2320 локального телнет-сервера с RFC2217. Это работает не только c dssh-сервером.<div id=11.14>
...
15.  `-s20 .` или короче `-s` как в [10.2](#10.2) + 127.0.0.1:2320 sshd-сервера X переносится на 127.0.0.1:2320 локального телнет-сервера с RFC2217.
