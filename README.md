...
# 4.    Как использовать для совместного доступа к последовательной консоли (или консоли с интерпретатором команд) нескольких клиентов:
    ...
    3.  Присоединяемся к сессии на порту 2322 командой `dssh -22 :` (присоединиться к сессии можно и `dssh -W2322 :`) или с вариантами параметров -u, -Z. Таких подключений может быть несколько.
    4.  Если клиенты находятся на хосте то присоединиться к сессии можно командой `dssh -22` или `dssh -22 .` (или как в 4.3 `dssh -W2322 .`) или `dssh -22 -j :`.
...
# 8.    Как ещё можно использовать dssh:
    ...
    5.  В 4.11 можно вместо интерпретатора команд указывать программу. Например `dssh -Htop :` это почти то же что и `dssh -t : top`. Вот только строка параметра -H не должна заканчиваться на цифру - иначе это будет принято за порт последовательной консоли. Например вместо `dssh -H"ping 8.8.8.8" :` надо `dssh -H"ping 8.8.8.8 "
