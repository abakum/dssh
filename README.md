...
17. Чтоб поделится локальной последовательной консолью через dssh-сервер `dssh -s22 :`. Для подключения к этой консоли `dssh -H:2322 :` или без управления `dssh -W2322 :` или без управления и без перевода консоли в сырой режим `ssh -W127.0.0.1:2322 :`, где алиас `:` это сокращение алиаса `ssh-j`.
...
1.  Если запустить на хосте `dssh` и проверив доступ `dssh :` потом к хосту можно подключится для удалённой разработки через `Remote - SSH extension` выбрав алиас `ssh-j` или ":" в `Connect to Host`.
